...
if [ -d "$ROOT_DIR/agents" ]; then
  while IFS= read -r -d '' agent_md; do
    ...
    name="$(basename "$dir")"
    description="$(awk 'NF && $0 !~ /^#/ {print; exit}' "$agent_md")"
    agents_json="$(jq -c --arg name "$name" --arg description "$description" --arg path "agents/$name/AGENTS.md" '. + [{name:$name,description:$description,path:$path}]' <<<"$agents_json")"
...
jq -n \
  ...
  '{generated_at:$generated_at,mcp:$mcp,skills:$skills,agents:$agents}' > "$ROOT_DIR/catalog.json"
