...
class Validator:
    ...
    def _check_components(self, plugin_dir: Path, manifest: dict) -> None:
        ...
        if isinstance(commands_value, list):
            for cmd_name in commands_value:
                cmd_path = self.repo / "commands" / f"{cmd_name}.md"
                if not cmd_path.is_file():
                    self.fail(f"{plugin_dir}: referenced command '{cmd_name}' not found at commands/{cmd_name}.md")
        else:
        ...
        if isinstance(mcp_value, list):
            for mcp_name in mcp_value:
                mcp_path = self.repo / "mcpServers" / mcp_name / ".mcp.json"
                if not mcp_path.is_file():
                    self.fail(f"{plugin_dir}: referenced mcpServer '{mcp_name}' not found at mcpServers/{mcp_name}/.mcp.json")
                else:
    ...
    def run(self) -> int:
        ...
        commands_dir = self.repo / "commands"
        if commands_dir.is_dir():
            for p in sorted(commands_dir.glob("*.md")):
                self.validate_standalone_command(p)
        mcp_dir = self.repo / "mcpServers"
        if mcp_dir.is_dir():
