...
def test_catalog_install_fields(fake_repo):
    ...
    assert mcp["install"]["vscodeMcpDeeplink"].startswith("https://vscode.dev/redirect/mcp/install?name=fixture-server&config=")
...
def test_catalog_list_ref_bundle_deeplink(fake_repo):
    ...
    assert bundle["install"]["vscodeMcpDeeplink"].startswith("https://vscode.dev/redirect/mcp/install?name=fixture-top-server&config=")
